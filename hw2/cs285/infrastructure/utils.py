...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while True:
        ...
        obs.append(ob)
        ac = policy.get_action(ob).sample()
        # ac = TODO # HINT: query the policy's get_action function
        acs.append(ac)

        # print(ac)
        # take that action and record results
        ob, rew, done, _ = env.step(ac.item())
        ...
        rollout_done = 1 if (done or len(rewards) > max_path_length) else 0
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    # paths = sample
    # print(len(paths))
    # print(paths[0]["action"], paths[0]["reward"])
